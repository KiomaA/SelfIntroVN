...
## 要顯示的文本。（who 參數可以為 None 如果沒有給出名字。）
##
## 此畫面必須建立 id 為 "what" 的可顯示文本，因為 Ren'Py 使用它來管理文字顯示。
...
screen say(who, what):
    ...
    ## 如果有側面圖像，請將其顯示在文字上方。不要顯示在手機版本上 - 沒有空間。
...
##
## 此畫面必須建立一個可顯示的輸入，通過 id "input" 以接受各種輸入參數。
##
...
## 遊戲選單畫面 ######################################################################
##
## 這列出了遊戲選單畫面的基本公共結構。它透過畫面標題進行調用，並顯示背景、標題
## 和導航。
##
## 滾動參數可以是None，或者是 "viewport" 與 "vpgrid" 的其中之一。此畫面旨在與一
    ...
    use game_menu(_("説明"), scroll="viewport"):
        ...
        vbox:
            ...
            hbox:
                ...
                textbutton _("鼠標") action SetScreenVariable("device", "mouse")

                if GamepadExists():
                    textbutton _("游戲手柄") action SetScreenVariable("device", "gamepad")
...
style skip_triangle:
    ## 我們必須使用帶有黑色右指小三角形字形的字體。
    font "DejaVuSans.ttf"


## 提醒畫面 ########################################################################
##
## 通知畫面用於向玩家顯示訊息。 （例如，當遊戲快速保存或截取螢幕截圖時。）
##
...
## 參數，必須創建一個 id 為 "what" 的可顯示內容，並且可以創建可顯示內容帶有
...
## 由於可能不存在滑鼠，我們將快捷選單替換為使用更少、更大、更容易觸摸的按鈕的版
## 本。
screen quick_menu():
    variant "touch"
